...
public class QuController extends BaseController {
    ...
    public ApiRest<BaseIdRespDTO> save(@RequestBody QuDetailDTO reqDTO) {
        if (StringUtils.isNotBlank(reqDTO.getContent()) && baseService.existsByContent(reqDTO.getContent())) {
            throw new ServiceException(1, "题目内容已存在，请勿重复添加：" + reqDTO.getContent());
        }
